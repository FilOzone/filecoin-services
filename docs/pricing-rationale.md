...
**Add pieces: base + per-piece.** Two terms for two gas components: fixed per-call overhead (base) and marginal per piece (calldata plus storage). `0.0005 + 0.0003 x N` tracks SP cost from one piece to a full batch. Batches currently cap at 40 pieces (FVM per-event size limit, independent of any contract limit), so `N` is bounded.
...
**Terminate service: 0.00112, user-initiated only.** Charged only when the client initiates (the SP earned it processing the wind-down). SP-initiated charges nothing and skips the reserve top-up: an SP walking away shouldn't drain client funds. Consent-based `terminateService` zeroes the PDP rail's lockup to terminate immediately; CDN rails persist until data-set deletion.

**Lifecycle reserve: how one-time fees are paid.** The PDP rail holds a small fixed-lockup pool (0.10 target, replenished below 0.005), so most ops cost one FilecoinPay interaction. FilecoinPay forbids raising a terminated rail's lockup, so the wind-down budget is whatever's in the pool at termination (user-initiated tops up first; SP-initiated doesn't); a client needing more wind-down deletions must pre-fund before terminating. Refunded at finalization if unused.
