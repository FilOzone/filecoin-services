...
Unexpected chain state is never silently copied into the manifest. When humans intend to retain it, an explicit component-scoped Adoption updates the manifest through normal PR review and may mark the state Unreproducible as an escape hatch. Pins remain managed state with justification and expiry rather than temporary rollout waivers; removing a component instead stops managing and verifying it without causing an on-chain action.
