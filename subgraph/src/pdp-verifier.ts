 ...
 * Adds pieces to a proof set and updates the provider's stats.
 */
export function handlePiecesAdded(event: PiecesAddedEvent): void {
  const dataSetId = event.params.dataSetId;
  const pieceIdsFromEvent = event.params.pieceIds; // Get piece IDs from event params
  ...
  // Decode dataSetId (uint256 at offset 0)
  let decodedSetId: BigInt = readUint256(encodedData, 0);
  if (decodedSetId != dataSetId) {
    log.warning(
      "Decoded dataSetId {} does not match event param {} in handlePiecesAdded. Tx: {}. Using event param.",
      [
 ...
 * Removes pieces from a proof set and updates the provider's stats.
 */
