...
contract ServiceProviderRegistryTest is MockFVMTest {
    ...
    function testInitialState() public view {
        // Check version
        assertEq(registry.VERSION(), "1.2.0", "Version should be 1.2.0");
    ...
    function testUpgradeV110ToV120LegacyBootstrapPreservesRegistryState() public {
        ServiceProviderRegistry oldImplementation = new ServiceProviderRegistry(2);

        ERC1967Proxy proxy =
            new ERC1967Proxy(address(oldImplementation), abi.encodeCall(ServiceProviderRegistry.initialize, ()));
        ServiceProviderRegistry productionRegistry = ServiceProviderRegistry(address(proxy));
        ...
        PDPOffering.Schema memory offering = PDPOffering.Schema({
            ...
            ipniPiece: true,
            ipniIpfs: false,
            storagePricePerTibPerDay: 123456789,
            minProvingPeriodInEpochs: 2880,
            location: "NO",
            paymentTokenAddress: IERC20(address(0))
        });
        (string[] memory capabilityKeys, bytes[] memory capabilityValues) = offering.toCapabilities(2);
        capabilityKeys[0] = "region";
        capabilityValues[0] = bytes("eu-north-1");
        capabilityKeys[1] = "tier";
        capabilityValues[1] = bytes("production");

        vm.deal(user1, 10 ether);
        vm.prank(user1);
        uint256 providerId = productionRegistry.registerProvider{value: 5 ether}(
            user2,
            "Production Provider",
            "Provider state must survive the registry implementation upgrade",
            ServiceProviderRegistryStorage.ProductType.PDP,
            capabilityKeys,
            capabilityValues
        );

        // Consume the same reinitializer version as the deployed v1.1.0 proxies. The current
        // implementation source is deliberately reused instead of copying historical contract code.
        productionRegistry.migrate("1.1.0");
        assertEq(_initializedVersion(address(productionRegistry)), 2, "pre-upgrade counter should be 2");

        address originalOwner = productionRegistry.owner();
        ServiceProviderRegistry newImplementation = new ServiceProviderRegistry(3);
        assertEq(newImplementation.VERSION(), "1.2.0", "new implementation should report v1.2.0");

        ServiceProviderRegistry.PlannedUpgrade memory plan = ServiceProviderRegistry.PlannedUpgrade({
            nextImplementation: address(newImplementation),
            afterEpoch: uint96(block.number + 240)
        });

        vm.expectEmit(false, false, false, true, address(productionRegistry));
        emit ServiceProviderRegistry.UpgradeAnnounced(plan);
        productionRegistry.announcePlannedUpgrade(plan);

        (address plannedImplementation, uint96 plannedAfterEpoch) = productionRegistry.nextUpgrade();
        assertEq(plannedImplementation, address(newImplementation), "legacy plan should target v1.2.0");
        assertEq(plannedAfterEpoch, plan.afterEpoch, "legacy plan should preserve the absolute epoch");

        bytes memory migrateData = abi.encodeCall(ServiceProviderRegistry.migrate, ("1.2.0"));
        vm.roll(plan.afterEpoch - 1);
        vm.expectRevert();
        productionRegistry.upgradeToAndCall(address(newImplementation), migrateData);

        vm.roll(plan.afterEpoch);
        vm.recordLogs();
        productionRegistry.upgradeToAndCall(address(newImplementation), migrateData);
        _assertRegistryUpgradeEvents(vm.getRecordedLogs(), address(productionRegistry), address(newImplementation));

        assertEq(
            _implementation(address(productionRegistry)),
            address(newImplementation),
            "proxy should use the v1.2.0 implementation"
        );
        assertEq(productionRegistry.VERSION(), "1.2.0", "proxy should report v1.2.0");
        assertEq(productionRegistry.owner(), originalOwner, "owner should survive the upgrade");
        assertEq(productionRegistry.getNextProviderId(), 2, "provider counter should survive the upgrade");
        assertEq(productionRegistry.activeProviderCount(), 1, "active provider count should survive the upgrade");
        assertEq(
            productionRegistry.productTypeProviderCount(ServiceProviderRegistryStorage.ProductType.PDP),
            1,
            "product count should survive the upgrade"
        );
        assertEq(
            productionRegistry.activeProductTypeProviderCount(ServiceProviderRegistryStorage.ProductType.PDP),
            1,
            "active product count should survive the upgrade"
        );
        assertEq(
            productionRegistry.getProviderIdByAddress(user1), providerId, "address lookup should survive the upgrade"
        );

        ServiceProviderRegistry.ServiceProviderInfoView memory provider = productionRegistry.getProvider(providerId);
        assertEq(provider.providerId, providerId, "provider ID should survive the upgrade");
        assertEq(provider.info.serviceProvider, user1, "provider owner should survive the upgrade");
        assertEq(provider.info.payee, user2, "provider payee should survive the upgrade");
        assertEq(provider.info.name, "Production Provider", "provider name should survive the upgrade");
        assertEq(
            provider.info.description,
            "Provider state must survive the registry implementation upgrade",
            "provider description should survive the upgrade"
        );
        assertTrue(provider.info.isActive, "provider active flag should survive the upgrade");

        ServiceProviderRegistryStorage.ProviderWithProduct memory product =
            productionRegistry.getProviderWithProduct(providerId, ServiceProviderRegistryStorage.ProductType.PDP);
        assertEq(uint256(product.product.productType), uint256(ServiceProviderRegistryStorage.ProductType.PDP));
        assertTrue(product.product.isActive, "product active flag should survive the upgrade");
        assertEq(product.product.capabilityKeys.length, capabilityKeys.length, "capability count should survive");
        assertEq(product.productCapabilityValues.length, capabilityValues.length, "capability values should survive");
        for (uint256 i = 0; i < capabilityKeys.length; i++) {
            assertEq(product.product.capabilityKeys[i], capabilityKeys[i], "capability key should survive the upgrade");
            assertEq(
                product.productCapabilityValues[i], capabilityValues[i], "capability value should survive the upgrade"
            );
        }
        assertEq(
            productionRegistry.productCapabilities(providerId, ServiceProviderRegistryStorage.ProductType.PDP, "region"),
            bytes("eu-north-1"),
            "capability mapping should survive the upgrade"
        );

        (plannedImplementation, plannedAfterEpoch) = productionRegistry.nextUpgrade();
        assertEq(plannedImplementation, address(0), "upgrade plan implementation should be cleared");
        assertEq(plannedAfterEpoch, 0, "upgrade plan epoch should be cleared");
        assertEq(_initializedVersion(address(productionRegistry)), 3, "migration should advance counter 2 to 3");

        vm.expectRevert(abi.encodeWithSignature("InvalidInitialization()"));
        productionRegistry.migrate("1.2.0");

        vm.expectRevert();
        productionRegistry.upgradeToAndCall(address(newImplementation), migrateData);
    }

    function _assertRegistryUpgradeEvents(Vm.Log[] memory logs, address emitter, address newImplementation)
        internal
        pure
    {
        bytes32 contractUpgradedTopic = keccak256("ContractUpgraded(string,address)");
        bytes32 initializedTopic = keccak256("Initialized(uint64)");
        bool foundContractUpgraded;
        bool foundInitialized;

        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].emitter != emitter || logs[i].topics.length == 0) continue;

            if (logs[i].topics[0] == contractUpgradedTopic) {
                (string memory version, address implementationAddress) = abi.decode(logs[i].data, (string, address));
                require(keccak256(bytes(version)) == keccak256(bytes("1.2.0")), "wrong upgrade event version");
                require(implementationAddress == newImplementation, "wrong upgrade event implementation");
                foundContractUpgraded = true;
            } else if (logs[i].topics[0] == initializedTopic) {
                require(abi.decode(logs[i].data, (uint64)) == 3, "wrong initialized event counter");
                foundInitialized = true;
            }
        }

        require(foundContractUpgraded, "ContractUpgraded event not found");
        require(foundInitialized, "Initialized(3) event not found");
    }
    ...
    function _initializedVersion(address proxy) internal view returns (uint64) {
        return uint64(uint256(vm.load(proxy, INITIALIZABLE_STORAGE_SLOT)));
    }
