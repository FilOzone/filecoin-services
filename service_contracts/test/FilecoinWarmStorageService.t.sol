...
contract FilecoinWarmStorageServiceTest is MockFVMTest {
    ...
    function testFilecoinServiceDeployedEvent() public {
        ...
        IFilecoinServiceMetadata metadata = IFilecoinServiceMetadata(address(newServiceProxy));

        Vm.Log[] memory entries = vm.getRecordedLogs();
        bytes32 eventSignature = keccak256("FilecoinServiceDeployed(string,string)");
        bool found;
        for (uint256 i = 0; i < entries.length; i++) {
            if (entries[i].topics.length == 1 && entries[i].topics[0] == eventSignature) {
                (string memory emittedName, string memory emittedDescription) =
                    abi.decode(entries[i].data, (string, string));
                assertEq(emittedName, metadata.name(), "Event name should match metadata");
                assertEq(emittedDescription, metadata.description(), "Event description should match metadata");
                found = true;
            }
        }
...
contract FilecoinWarmStorageServiceUpgradeTest is Test {
    ...
    function testUpgradeV130ToV131LegacyBootstrapPreservesMainnetState() public {
        _testUpgradeV130ToV131LegacyBootstrap(4, 5);
    }

    function testUpgradeV130ToV131LegacyBootstrapPreservesCalibnetState() public {
        _testUpgradeV130ToV131LegacyBootstrap(8, 9);
    }

    function _testUpgradeV130ToV131LegacyBootstrap(uint64 oldCounter, uint64 newCounter) internal {
        FilecoinWarmStorageService oldImplementation = _deployImplementation(oldCounter);
        _replaceRuntimeString(address(oldImplementation), bytes("1.3.1"), bytes("1.3.0"));

        bytes memory initData =
            abi.encodeCall(FilecoinWarmStorageService.initialize, (uint64(2880), uint256(60), filBeamController));
        MyERC1967Proxy proxy = new MyERC1967Proxy(address(oldImplementation), initData);
        FilecoinWarmStorageService service = FilecoinWarmStorageService(address(proxy));
        FilecoinWarmStorageServiceStateView stateView = new FilecoinWarmStorageServiceStateView(service);

        assertEq(service.VERSION(), "1.3.0", "fixture should model the deployed v1.3.0 implementation");

        service.setViewContract(address(stateView));
        service.configureProvingPeriod(4320, 120);
        service.addApprovedProvider(101);
        service.addApprovedProvider(202);

        // Prime the proxy to the reinitializer counter currently used by the deployed v1.3.0 instance.
        service.migrate(address(0));
        assertEq(_initializedVersion(address(service)), oldCounter, "pre-upgrade counter should match production");

        address originalOwner = service.owner();
        address originalView = service.viewContractAddress();

        FilecoinWarmStorageService newImplementation = _deployImplementation(newCounter);
        assertEq(newImplementation.VERSION(), "1.3.1", "new implementation should report v1.3.1");

        FilecoinWarmStorageService.PlannedUpgrade memory plan = FilecoinWarmStorageService.PlannedUpgrade({
            nextImplementation: address(newImplementation),
            afterEpoch: uint96(block.number + 240)
        });

        vm.expectEmit(false, false, false, true, address(service));
        emit FilecoinWarmStorageService.UpgradeAnnounced(plan);
        service.announcePlannedUpgrade(plan);

        (address plannedImplementation, uint96 plannedAfterEpoch) = stateView.nextUpgrade();
        assertEq(plannedImplementation, address(newImplementation), "legacy plan should target v1.3.1");
        assertEq(plannedAfterEpoch, plan.afterEpoch, "legacy plan should preserve the absolute epoch");

        bytes memory migrateData = abi.encodeCall(FilecoinWarmStorageService.migrate, (address(0)));
        vm.roll(plan.afterEpoch - 1);
        vm.expectRevert();
        service.upgradeToAndCall(address(newImplementation), migrateData);

        vm.roll(plan.afterEpoch);
        vm.expectEmit(false, false, false, true, address(service));
        emit FilecoinWarmStorageService.ContractUpgraded("1.3.1", address(newImplementation));
        service.upgradeToAndCall(address(newImplementation), migrateData);

        assertEq(_implementation(address(service)), address(newImplementation), "proxy should use v1.3.1");
        assertEq(service.VERSION(), "1.3.1", "proxy should report v1.3.1");
        assertEq(service.owner(), originalOwner, "owner should survive the upgrade");
        assertEq(service.viewContractAddress(), originalView, "view contract should survive the upgrade");
        assertEq(service.pdpVerifierAddress(), address(mockPDPVerifier), "PDP verifier should be unchanged");
        assertEq(service.paymentsContractAddress(), address(payments), "payments contract should be unchanged");
        assertEq(address(service.usdfcTokenAddress()), address(mockUSDFC), "USDFC token should be unchanged");
        assertEq(service.filBeamBeneficiaryAddress(), filBeamBeneficiary, "FilBeam beneficiary should be unchanged");
        assertEq(
            address(service.serviceProviderRegistry()), address(serviceProviderRegistry), "SPR should be unchanged"
        );
        assertEq(
            address(service.sessionKeyRegistry()), address(sessionKeyRegistry), "session registry should be unchanged"
        );

        (uint64 maxProvingPeriod, uint256 challengeWindowSize,,) = stateView.getPDPConfig();
        assertEq(maxProvingPeriod, 4320, "max proving period should survive the upgrade");
        assertEq(challengeWindowSize, 120, "challenge window should survive the upgrade");
        assertEq(stateView.getApprovedProvidersLength(), 2, "approved provider count should survive the upgrade");
        uint256[] memory approvedProviders = stateView.getApprovedProviders(0, 10);
        assertEq(approvedProviders.length, 2, "approved provider list should survive the upgrade");
        assertEq(approvedProviders[0], 101, "first approved provider should survive the upgrade");
        assertEq(approvedProviders[1], 202, "second approved provider should survive the upgrade");

        (plannedImplementation, plannedAfterEpoch) = stateView.nextUpgrade();
        ...
        assertEq(_initializedVersion(address(service)), newCounter, "migration should advance the counter once");
    ...
    function _deployImplementation(uint64 reinitializerVersion)
        internal
        returns (FilecoinWarmStorageService implementation)
    {
        implementation = new FilecoinWarmStorageService(
            address(mockPDPVerifier),
            address(payments),
            mockUSDFC,
            filBeamBeneficiary,
    ...
    function _replaceRuntimeString(address target, bytes memory from, bytes memory to) internal {
        require(from.length == to.length, "replacement length mismatch");
        bytes memory runtime = target.code;
        bool replaced;

        for (uint256 i = 0; i + from.length <= runtime.length; i++) {
            bool matches = true;
            for (uint256 j = 0; j < from.length; j++) {
                if (runtime[i + j] != from[j]) {
                    matches = false;
                    break;
                }
            }
        ...
        assertTrue(replaced, "version literal not found in implementation runtime");
