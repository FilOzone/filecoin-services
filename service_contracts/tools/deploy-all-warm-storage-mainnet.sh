#! /bin/bash
# deploy-all-warm-storage-mainnet deploys the PDP verifier, Payments contract, and Warm Storage service to mainnet
...
echo "Deploying all Warm Storage contracts to mainnet"
...
if [ "$CHAIN_ID" != "314" ]; then
  echo "Error: This script is for Filecoin mainnet only"
  echo "  Expected chain ID: 314 (mainnet)"
  echo "  Detected chain ID: $CHAIN_ID"
  exit 1
fi

echo "Detected Chain ID: $CHAIN_ID (mainnet)"
...
if [ -z "$FILCDN_CONTROLLER_ADDRESS" ]; then
    FILCDN_CONTROLLER_ADDRESS="0x5f7E5E2A756430EdeE781FF6e6F7954254Ef629A" # FilCDN Controller address
fi
if [ -z "$FILCDN_BENEFICIARY_ADDRESS" ]; then
    FILCDN_BENEFICIARY_ADDRESS="0xff0000000000000000000000000000000037385c" # FilCDN Beneficiary address
fi
USDFC_TOKEN_ADDRESS="0x80B98d3aa09ffff255c3ba4A241111Ff1262F045"    # USDFC Mainnet token address

# Proving period configuration - use defaults if not set
MAX_PROVING_PERIOD="${MAX_PROVING_PERIOD:-2880}"                    # Default 2880 epochs
CHALLENGE_WINDOW_SIZE="${CHALLENGE_WINDOW_SIZE:-480}"                # Default 15 epochs
