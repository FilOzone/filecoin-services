...
- `service_contracts/deployments.json` on a release branch or tag is a point-in-time snapshot and may be stale after proxy or View switches. Update `deployments.json` on `main` through the follow-up PR flow, but treat chain state and linked execute transactions as the live verification source.
