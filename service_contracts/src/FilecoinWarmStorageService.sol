...
contract FilecoinWarmStorageService is PDPListener, IArbiter, Initializable, UUPSUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function dataSetCreated(uint256 dataSetId, address creator, bytes calldata extraData) external onlyPDPVerifier {
        ...
        uint256 railId = payments.createRail(
            ...
            creator, // dataset creator, SPs in  most cases
        ...
        // This is a payment from payer to dataset creator of a fixed amount
    ...
    function piecesAdded(
        uint256 dataSetId,
        uint256 firstAdded,
        PDPVerifier.PieceData[] memory pieceData,
        bytes calldata extraData
    ) external onlyPDPVerifier {
        // Verify the data set exists in our mapping
        DataSetInfo storage info = dataSetInfo[dataSetId];
    ...
    /**
     * @notice Handles data set ownership changes by updating internal state only
     * @dev Called by the PDPVerifier contract when data set ownership is transferred. This function is now fully decoupled from the provider registry.
     * @param dataSetId The ID of the data set whose ownership is changing
    ...
    function ownerChanged(
        ...
        // Verify the data set exists and validate the old owner
        DataSetInfo storage info = dataSetInfo[dataSetId];
    ...
    function verifyAddPiecesSignature(
        address payer,
        uint256 clientDataSetId,
        PDPVerifier.PieceData[] memory pieceDataArray,
        uint256 firstAdded,
        bytes memory signature
    ) internal view returns (bool) {
        ...
        for (uint256 i = 0; i < pieceDataArray.length; i++) {
            ...
            bytes32 cidHash = keccak256(
                abi.encode(
                    CID_TYPEHASH,
                    keccak256(pieceDataArray[i].pieceCID.data)
                )
            );
