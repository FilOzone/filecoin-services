...
**Immediate termination (consent case)**: When the SP calls `terminateService(dataSetId, extraData)` with a valid payer EIP-712 signature, FWSS attempts to set `lockupPeriod = 0` before calling `terminateRail`. For well-funded payers (`isAccountLockupFullySettled` is true), this succeeds: `endEpoch = block.number` and the payer's streaming buffer is released immediately. For underfunded payers, FilecoinPay blocks the lockup-period change and the rail enters the standard 30-day window instead. In either case the terminate fee is charged and any remaining lifecycle reserve is released at the next settlement.
